...
const PersonalLibraryQuery = {
    ...
    if(user.role == 'admin'){
        throw new GraphQLError("Admins are not allowed to have a personal library. Use a different account");
    }
    try{
        const library = await db.User.findOne({ where: { id: user.id },
            include: [{
                model: db.Book,
                as: 'books'   
            }]
        ...
        library.books = library.books.slice(((page - 1) * nrBooksOnPage), (page * nrBooksOnPage));
        return library;
    }
    catch(exception){
